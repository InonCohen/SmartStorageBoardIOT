...
MFRC522 mfrc522(SS_PIN_RC522, RST_PIN_RC522); // Create instance of MFRC522
  ...
  void printUnlockMessage(char* name)
  {
    display.display();
...
void setup(){
    ...
    display.setTextSize(TEXT_SIZE);
    display.setCursor(10,0); 
    display.print("Approximate your card to the reader...");
    display.display();
...
void loop(){
  ...
  while(readLine(file, file_content, file.size())){
    ...
    if (content.substring(1) == userID) //change here the UID of the card/cards that you want to give access
    {
      ...
      file_content += (strlen(userID)+1);
      char* userName = strtok(file_content, ",");
      printUnlockMessage(userName);
      authorized = true;
      Serial.println();
      delay(3000);
      break;
