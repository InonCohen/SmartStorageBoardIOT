...
bool readLine(fs::FS &fs, const char * path, char* line, size_t maxLen) {
...
void initSD()
{
    ...
    clearSD();
    listDir(SD, "/", 0);
    writeFile(SD, "/users.csv", "initialize: \n");
    writeFile(SD, "/items.csv", "initialize: \n");
...
void loop(){
  ...
  readLine(SD, "/users.csv", file_content, file.size());
  char* usersID = (char*)malloc(100);
  usersID = parser(file_content);
  Serial.print(usersID);
